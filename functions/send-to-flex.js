...
exports.handler = async function (context, event, callback) {
  ...
  try {
    ...
    const workerSid =
      syncDoc.data.caseDetails.workerDetails[0].Flex_WorkerId__c;
    ...
    const caseCommentCreated =
      caseId && conn
        ? await createCaseComment(conn, caseId, "CaseComment", summary)
        : null;
    ...
    const FLEX_WORKFLOW_SID =
      event.FlexWorkflowSid || context.FLEX_WORKFLOW_SID;
    ...
    const [serviceSid, conversationsSid] = event.request.headers["x-session-id"]
      ...
      .split("/");
    const [traitName, identity] =
      event.request.headers["x-identity"]?.split(":");
    ...
    console.error(
      "There was an error during the transfer. Attempting to transfer without additional info.",
      err
    );

    if (sessionId.startsWith("voice")) {
      console.log("Forwarding call to Studio Flow without details");
      const redirectUrl = `https://webhooks.twilio.com/v1/Accounts/${context.ACCOUNT_SID}/Flows/${context.STUDIO_FLOW_SID}?FlowEvent=return`;
      console.log("Redirect URL: ", redirectUrl);
      const [callSid] = sessionIdOrig.replace("voice:", "").split("/");
      console.log("Call SID: ", callSid);
      await new Promise((resolve) => setTimeout(resolve, 1000));
